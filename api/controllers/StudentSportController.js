...
module.exports = {
  ...
  getContingentStrength: function(req, res) {
    if (req.body) {

        if(req.body.year && req.body.school && objectid.isValid(req.body.school)){
      StudentSport.getContingentStrength(req.body, function(err, respo) {
        if (err) {
          res.json({
            value: false,
            data: err
    ...
    StudentSport.aggregate([{
      ...
      Student.populate(response, [{
        ...
        if (err) {
          ...
          //   }
          // });
          Config.generateExcel("Students ALL",excelData,res);
        }
      });
    });
  }
